...
exports.MinecraftServerListPing = void 0;
...
class MinecraftServerListPing {
    ...
    static ping(protocol = 4, host, port = 25565, timeout = 3000) {
        return new Promise((resolve, reject) => {
            ...
            client.on("connect", () => {
                ...
                // Protocol Version
                const protocolBuffer = Buffer.from(varint_1.default.encode(protocol));
                // Server Address
                const hostLengthBuffer = Buffer.from(varint_1.default.encode(host.length));
                ...
                const dataLengthBuffer = Buffer.from(varint_1.default.encode(dataBuffer.length));
            ...
            client.on("data", (responseBuffer) => {
                ...
                // Error: Invalid
                try {
                    responseDataBufferLength = varint_1.default.decode(responseDataBuffer);
                }
                catch (error) {
                    return;
                }
                // Error: Too short
                if (responseDataBuffer.length < responseDataBufferLength - varint_1.default.decode.bytes)
                    return;
                let offset = varint_1.default.decode.bytes;
                // Packet ID
                varint_1.default.decode(responseDataBuffer, offset);
    ...
    static ping16(protocol = 73, host, port = 25565, timeout = 3000) {
        return new Promise((resolve, reject) => {
            ...
            client.on("connect", () => {
                ...
                const hostBuffer = iconv_lite_1.default.encode(host, "utf16-be");
            ...
            // Response
            client.on("data", (responseBuffer) => {
                const dataBuffer = iconv_lite_1.default.decode(responseBuffer.slice(9, responseBuffer.length), "utf-16be");
    ...
    static ping13(host, port = 25565, timeout = 3000) {
        return new Promise((resolve, reject) => {
            ...
            // Response
            client.on("data", (responseBuffer) => {
                const dataBuffer = iconv_lite_1.default.decode(responseBuffer.slice(3, responseBuffer.length), "utf-16be");
...
exports.MinecraftServerListPing = MinecraftServerListPing;
